...
class  MasterOne(object):
	...
	def write_master_list(self, companies = None):
		...
		for property_ in properties:
			external_id=str(property_[0]).replace('.0','').replace('.00','').strip()
			status=property_[1]
			company=property_[2]		
			m=property_[3]
			title=property_[4]
			address=property_[5]
			unit=property_[6].replace('.0','')
			city=property_[7]
			state=property_[8]
			...
			# cursor.execute("SELECT * FROM master_csv WHERE external_id::text=%s AND company=%s",
			# 	           (str(external_id).strip(),str(company).strip(),))
			# exist=cursor.fetchone()
			# if exist:
			# 	cursor.execute("UPDATE master_csv SET status=%s WHERE company=%s AND title=%s AND address=%s AND unit=%s",
			# 	           (status,company,title,address,unit,))
		
			# else:

			cursor.execute("UPDATE master_csv SET status=%s,m=%s,title=%s,\
									address=%s,unit=%s,zip_code=%s,\
									square_feet=%s,forms=%s,parking_space=%s,area=%s,type=%s,\
							        bedroom=%s,bathroom=%s,monthly_rent=%s,price_bedroom=%s,available=%s,\
							        paid_utilities=%s,pets=%s,pets_info=%s,laundry=%s,web_description=%s,\
							        tenant_phone=%s,gate_code=%s,\
							        key_number=%s,email=%s,commission=%s,bonus=%s,deposit_paid_to=%s,\
							        description=%s,double_occupancy=%s,dbl_occ_info=%s,listing_type=%s,pool=%s WHERE external_id::text=%s",
							      (status,m,title,
									address,unit,zip_code,
							        ...
							        description,double_occupancy,dbl_occ_info,listing_type,pool,external_id,))
			# else:
